...
%DIF DEL first_draft.tex   Tue Apr 27 11:11:28 2021
%DIF ADD main.tex          Tue May  4 17:45:51 2021
%DIF < \documentclass[journal=jmcmar,manuscript=article]{achemso}
%DIF -------
\documentclass[journal=jcim,manuscript=applicationnotes]{achemso} %DIF > 
%DIF -------
...
\DIFaddbegin \DIFadd{AqSolDB spans a wide range of solubility values (Fig~\ref{fig:esol_hist}) and is collated from differing datasets, most of which controlled for temperature.
...
Additionally, from our previous work\mbox{%DIFAUXCMD
\cite{cd2020}}\hspace{0pt}%DIFAUXCMD
, we have observed that neural network models tend to perform better with larger datasets, even if the data contains more noise.
Thus, we elected to utilize AqSolDB for our training set over the smaller ESOL\mbox{%DIFAUXCMD
\cite{esol} }\hspace{0pt}%DIFAUXCMD
which was used in the original MAT publication\mbox{%DIFAUXCMD
\cite{MAT}}\hspace{0pt}%DIFAUXCMD
, a comparative analysis of which can be found in Fig~\ref{fig:stndepesol}-\ref{fig:stnesolaqsol} and Table~\ref{tab:esolaqsol}.
}\DIFaddend Notably, while there are many other features available in AqsolDB, we only utilized the included SMILES strings and the reported solubilities (logS, S in mol/L).
...
We additionally created \DIFdelbegin \DIFdel{an independent }\DIFdelend \DIFaddbegin \DIFadd{a withheld }\DIFaddend test set out of the molecules present in the SC2 test sets \cite{llinas} and FreeSolv \cite{freesolv}, such that no molecule in the \DIFdelbegin \DIFdel{independent }\DIFdelend \DIFaddbegin \DIFadd{withheld }\DIFaddend test set has an RDkit fingerprint similarity of greater than or equal to 0.9 to any molecule in AqSolDB.
The histograms of solubility values for each fold of the scaffold split, the full AqSolDB, and our \DIFdelbegin \DIFdel{independent }\DIFdelend \DIFaddbegin \DIFadd{withheld }\DIFaddend set are shown in Fig~\ref{fig:solhists}.
...
\DIFaddbegin \DIFadd{We also note that our molecular embedding layer calculates the features for the molecular graph from the RDKit's molecule object of each SMILES, which ensures consistency of results across different SMILES representations.
}\DIFaddend In order to select the final deployed model, we trained 5 models with different random seeds on all of AqSolDB utilizing the Huber loss function, the SGD optimizer with momentum of 0.06, no weight decay, and a learning rate of 0.04.
We dynamically stopped training after performance on the training set stopped improving for 8 epochs.
After which we selected the model architecture with the best average RMSE performance on our \DIFdelbegin \DIFdel{independent }\DIFdelend \DIFaddbegin \DIFadd{withheld }\DIFaddend test set and selected the best performing model from within that architecture class (Fig~\ref{fig:dynsweep}).
\DIFaddbegin \DIFadd{The dropout in the final model and our early stopping criteria both help to prevent overtraining of the deployed SolTranNet.
}\DIFaddend 
...
We selected \DIFdelbegin \DIFdel{as }\DIFdelend the top RMSE performers of each class of models (by number of parameters) to show.
...
We focus on the RMSE metric, since the large range of true \DIFdelbegin \DIFdel{solibilities }\DIFdelend \DIFaddbegin \DIFadd{solubilities }\DIFaddend makes it easier to achieve a higher Pearson $R^2$ (Table~\ref{tab:solsearchr2}).

We quantified SolTranNet's generalization by training 5 different models on all of AqSolDB and testing them on our withheld \DIFdelbegin \DIFdel{independent }\DIFdelend set (Table~\ref{tab:deployed}).
...
\DIFaddbegin \DIFadd{A much computationally cheaper process would be to utilize a 2D conformer of the molecule generated by RDkit, which is the behavior of the original MAT code if a given 3D conformer could not be computed.
}\DIFaddend We show that using a 2D or 3D conformer does not have a statistically significant difference on RMSE or $R^2$ for the first 10 hyper-parameter combinations of SolTranNet during the architecture sweep ($p=0.144$) (Figure~\ref{fig:2dv3d}).
Thus, we used distances determined \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{via RDKit's Compute2DCoords function with default parameters as the }\DIFaddend 2D \DIFdelbegin \DIFdel{conformers }\DIFdelend \DIFaddbegin \DIFadd{conformer }\DIFaddend for the remainder of the sweep. 
...
These two groups overlap by 75.2\% and 83.9\% respectively; that is, unusual atom types are typically due to salts\DIFaddbegin \DIFadd{, typically in the counter ion}\DIFaddend .
Thus, we investigate the effect of fragmenting salts (\DIFdelbegin \DIFdel{only }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend keeping the largest component\DIFaddbegin \DIFadd{, thus removing the counter ion}\DIFaddend ) and removing successively larger molecules with ``Other'' typed atoms from the training data (Figure~\ref{fig:saltfragrmse},\ref{fig:saltfragr2}) \DIFaddbegin \DIFadd{in order to gain an understanding of the effect rare atom types have on model performance}\DIFaddend .
...
We train on the full salt since it allows less \DIFdelbegin \DIFdel{prepossessing }\DIFdelend \DIFaddbegin \DIFadd{preprocessing }\DIFaddend for the user.
...
To benchmark SolTranNet, we determined the mean time for a solubility prediction from \DIFdelbegin \DIFdel{a SMILES string }\DIFdelend \DIFaddbegin \DIFadd{SMILES strings }\DIFaddend for 1,000 molecules (repeats of the 132 SC2 molecules).
...
SolTranNet, with fewer \DIFdelbegin \DIFdel{number of }\DIFdelend parameters and skipping conformer generation, runs 2.3 times faster on GPU (12.0 times faster on CPU) than MAT utilizing the 2D conformer generation.
...
In order to compare SolTranNet to other methods, we \DIFaddbegin \DIFadd{first verified SolTranNet's performance on ESOL\mbox{%DIFAUXCMD
\cite{esol} }\hspace{0pt}%DIFAUXCMD
as in the MAT publication\mbox{%DIFAUXCMD
\cite{MAT}}\hspace{0pt}%DIFAUXCMD
, and then }\DIFaddend evaluated SolTranNet on four recently published solubility training and testing sets\cite{lovric,boobier,cui,llinas} (Table~\ref{tab:othersetsrmse},\ref{tab:othersetsr2}).
...
However, for the datasets provided by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{lovric} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{ESOL\mbox{%DIFAUXCMD
\cite{esol}}\hspace{0pt}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\citet{lovric}}\hspace{0pt}%DIFAUXCMD
, }\DIFaddend and \citet{boobier}, this performed \DIFdelbegin \DIFdel{very }\DIFdelend poorly and stopped training early \DIFdelbegin \DIFdel{, }\DIFdelend due to their smaller size  (\DIFaddbegin \DIFadd{1128, }\DIFaddend 530 and 75 molecules respectively) as compared to our AqSolDB training sets (6655 CCV and 9982 full).
As such, we trained new models for 1000\DIFaddbegin \DIFadd{, 1000, }\DIFaddend and 500 epochs respectively for those sets.
\DIFaddbegin \DIFadd{Reassuringly, our implementation of the original MAT architecture and SolTranNet exhibit no statistically significant difference in performance when training with the provided ESOL random splits (p=0.073 and p=0.26 respectively).
...
}\DIFaddend For the datasets provided by \citet{cui}, \citet{lovric}, and \citet{boobier}, SolTranNet's best performing model trained on the provided datasets achieves similar results to the method reported in the respective paper.
...
\begin{table}
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
        \hline
        Dataset &  Reported & \DIFdelbeginFL \DIFdelFL{STN }\DIFdelendFL Training &  \DIFdelbeginFL \DIFdelFL{STN }\DIFdelendFL Best & \DIFdelbeginFL \DIFdelFL{STN }\DIFdelendFL Deployed & Overlap \\
        \hline
        \DIFaddbeginFL \DIFaddFL{ESOL MAT }& \DIFaddFL{0.278 (0.20) }& \DIFaddFL{0.306 (0.027) }& \DIFaddFL{-- }& \DIFaddFL{-- }& \DIFaddFL{1128/1128 }\\
        \DIFaddFL{ESOL STN }& \DIFaddFL{0.278 (0.20) }& \DIFaddFL{0.289 (0.011) }& \DIFaddFL{-- }& \DIFaddFL{2.99 (0.108) }& \DIFaddFL{1119/1128 }\\
        \hline
        \DIFaddendFL Cui2020 & 0.681 & 0.860 (0.215) & 0.624 & 0.813 & 0/62 \\
        ...
        \hline
    \end{tabular}
    \caption{RMSE performance of SolTranNet (STN) on other published datasets. \DIFaddbeginFL \DIFaddFL{The first two rows are testing the original MAT implementation on ESOL, and then testing SolTranNet's implementation. For both the original MAT and SolTranNet implementation, there is no difference between our training and the reported result (p=0.073 and p=0.26 respectively). }\DIFaddendFL For SolTranNet Training, we trained five different seeds of our final architecture on the provided training and testing splits and report the mean and standard deviation (in parentheses). The SolTranNet Deployed column is using our final deployed model to predict the provided test set. The final column is the overlap of the provided test set with our deployed model's training set\DIFaddbeginFL \DIFaddFL{, since there was no attempt to remove molecules present in the test set from the training set}\DIFaddendFL . Notably the Lovric set had five different randomly selected splits for training and testing, which is why there is a mean in the Deployed and Overlap columns.}
...
\DIFaddbegin \DIFadd{This is why we selected our final architecture by its performance on our withheld test set, and why models with dropout tended to perform better as both of these techniques help to reduce overfitting to the training set.
}\DIFaddend We hypothesize that the smaller models are more suited to solubility prediction until more training data becomes available.
...
\DIFaddbegin \subsection{\DIFadd{Data and Software}}
\DIFadd{SolTranNet is open source and available under the Apache2.0 license. SolTranNet is available via }\textsc{\DIFadd{pip}} \DIFadd{installation for }\textsc{\DIFadd{python3}}\DIFadd{, and the source code is available at }\url{https://github.com/gnina/SolTranNet}\DIFadd{. All datasets and code used for the analysis in this paper is available at }\url{https://github.com/francoep/SolTranNet_paper}

\subsection{\DIFadd{Funding Sources}}
\DIFaddend This work is supported by R01GM108340 from the National Institute of General Medical Sciences and a GPU donation from the NVIDIA corporation.
